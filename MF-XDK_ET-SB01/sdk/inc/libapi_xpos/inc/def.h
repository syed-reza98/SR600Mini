...
typedef struct
{
	int     DialMode;        ///< Dial mode:0 Synchronize,1 Asynchronous
...
typedef struct
{
	char cLen;			///<card number len 
	char szCardNo[10];	///<card number (hex)  eg "\x62\x25\x75\x21\x80\x39\x94\x63"
	char cIndex;		///<index
}BlackCard;
...
#define  UMAX_TERMINAL_APPL    128        ///< The maximum number of applications supported by card terminals
...
typedef struct
{
	...
	byte abTrnCurrencyCode[2];			///< Currency code tag: 5F2A // Does not support now
	...
	byte cAidFileType;					///< Two modes (0---default) or (1--contact 2--contactless)
	byte szOtherTLV[256];				// other tlv setting eg: "\x9F\x6D\x01\xC0\x9F\x6E\x04\xD8\xE0\x00\x00"  9f6d AND 9f6efor amex
	byte cOthertlvLen;					//length of other tlv eg: length of "\x9F\x6D\x01\xC0\x9F\x6E\x04\xD8\xE0\x00\x00" is 11
	byte cKernelID;					// 0x02->mastercard  0x03->visa 0x04->amex 0x05->JCB 0x06->discover 0x07-unionpay 0x09->pure
}APPLICATIONPARAMS;
...
typedef struct
{
	unsigned char ucCOMMType;  ///< Communication type:See enum UTMSCOMTYPE
