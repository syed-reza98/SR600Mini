...
LIB_EXPORT int Icc_Get_Magstripe_Data(E_GAC genactype,const char *psData,int nDataLen, char * pszOut, int * nOutLen);
LIB_EXPORT int Icc_Echo(char * pszOut, int * nOutLen);
LIB_EXPORT int Icc_Put_Data_For_PURE(char *pszTag,const char *psData,int nDataLen,char * psResponse,int *pnDataLen);
LIB_EXPORT int Icc_ECO_CMD_For_PURE(char cGACflg,const char *psData,int nDataLen,char * psResponse,int *pnDataLen);
LIB_EXPORT int Icc_Perform_Transaction(const char* psData, int nDataLen, char* psResponse, int* pnDataLen);	//mir
LIB_EXPORT int Icc_Complete_Transaction(const char* psData, int nDataLen, char* psResponse, int* pnDataLen);			//mir

LIB_EXPORT int Icc_data_Get_Progress_Op(const char *psData,int nDataLen,char * psResponse,int *pnDataLen,char p1);
LIB_EXPORT int Icc_Read_Record_DS( char cSfi,const char cRecordNo,char * psResponse,int *pnDataLen);
LIB_EXPORT int Icc_Resume_Get_Progress_Op(const char *psData,int nDataLen,char * psResponse,int *pnDataLen,char p1);
LIB_EXPORT int Icc_Put_Data_Dpas(char *pszTag,const char *psData,int nDataLen,char * psResponse,int *pnDataLen);
LIB_EXPORT int Icc_Put_Data_Dpas_sign(char *pszTag,const char *psData,int nDataLen,char * psResponse,int *pnDataLen);
