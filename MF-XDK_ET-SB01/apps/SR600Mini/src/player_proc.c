...
//{"broadcast_type":99,"voiceArray":[{"name":"num1","type":"01"},{"name":"num2","type":"01"},{"name":"num3","type":"01"}]}
int PlayVoiceArray(cJSON *rootobj)
{
	...
	if ( json_getval(rootobj,"broadcast_type" , temp, sizeof(temp)) > 0 )
	{
		...
		if (broadcast_type == 99)
		{
			cJSON *array = cJSON_GetObjectItem(rootobj, "voiceArray");
			if (array)
			{
				...
				for (int i=0; i<cnt; i++)
				{
					...
					{
						cJSON *type = cJSON_GetObjectItemCaseSensitive(arraytemp, "type");
						if (type)
						{
							APP_TRACE("type value: %s\r\n", type->valuestring);
							SetLanguage(atoi(type->valuestring));
						}
...
void voice_play_func(cJSON* rootobj)
{
		...
		&& json_getval(rootobj, "money", strmoney, sizeof(strmoney)) >= 0 )
	{
		...
		if (broadcast_type == 1)
		{
			APP_TRACE("show_page_result broadcast_type=%d\r\n", broadcast_type);
			...
			PayClosePage();
			page_show_image(get_mainpage(), NULL, "QRCODE", 0, 0, 3000);
			//set_counter_led();
		}
...
#define QRCODE_MAXLENTH	(512)
void on_user_update(const char *payload, int len)
{
	char temp[256] = {0};
	char qrcode[QRCODE_MAXLENTH+1] = {0};
	cJSON* rootobj = NULL;
    ...
    if(json_getval(rootobj, "order_sn", qrcode, QRCODE_MAXLENTH) >= 0 )
	{    
		...
		{
			show_page_qrcode(get_mainpage(), NULL, temp, qrcode, 60*1000);
		}
	}
	else
	{
...
int play_proc_with_fifo()
{
	...
	if (payload != NULL)
	{
		...
		if (payloadlen > 0)
		{
			...
			PayPagePowerUnlock("func_pay play");
		}
	...
	if (m->payloadlen > 0)
	{
		...
		if (json_getval(rootobj, "request_id", messageId, sizeof(messageId)) > 0)
		{
			...
			if (0 == check_messageId(messageId)) 
			{
				...
				if(json_getval(rootobj, "money", amount, sizeof(amount)) > 0)
				{
					//message feedback to test platform.
					//Test environment functionality, please make changes according to the requirements of the connected mqtt platform
