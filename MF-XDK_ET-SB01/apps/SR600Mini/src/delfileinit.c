...
void delfile_init()
{
	...
	MfSdkFsReadProfileString((s8*)DELMP3_INI_SECTION,(const s8*)"num", (s8*)temp, sizeof(temp),(const s8*)"",(s8*)DELMP3_INI_FILENAME);
	...
	for (i = 0; i < num; i++)
	{
		...
		MfSdkFsReadProfileString((s8*)DELMP3_INI_SECTION,(const s8*)key, (s8*)val, sizeof(val),(s8*)"", (s8*)DELMP3_INI_FILENAME);
