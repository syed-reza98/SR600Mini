...
static int AppInputOnlinePinEx(char* psCardNo, char* psAmt, char* psCiphertext)
{
	...
	//APP_TRACE("AppInputOnlinePinEx sPin:%s\r\n",sPin);
...
static int AppInputPinEx(char* psCardNo, char* psAmt, char cMsgType, char* psPin) /*输入密码界面函数指针*/
{
	...
	lv_obj_t* emv_page_win = app_emv_get_page_win();
	if(NULL == emv_page_win) { emv_page_win = emv_get_page_win(); }

    if(emv_page_win)
	{
        //page_input_show_ex(l_emv_page_win, input_pin_return_func, "Sale", password, 4, 12, PAGE_INPUT_MODE_PWD, OK_TIME_OVER, 0);
		LV_THREAD_LOCK(page_input_show_psw(emv_page_win, input_pin_return_func, get_card_title(), "input offline PIN", password, 4, 12, PAGE_INPUT_MODE_PWD, 60000));
    }
    else
    {
...
static int g_secondTapRet = 999;

static int AppEmvSecondTapCardCallback(int ret, lv_obj_t* obj)
{
    APP_TRACE("AppEmvSecondTapCardCallback:%d", ret);
    g_secondTapRet = ret;
    return g_secondTapRet;
}
 ...
 * @return 
 */
static s32 AppEmvSecondTapCard(void)
{
	int iRet = -7;
	lv_obj_t* emv_page_win = NULL;
	...
	lv_start_lock(1);
	emv_page_win = app_emv_get_page_win();
	if(NULL == emv_page_win) { emv_page_win = MfSdkEmvGetPageWin(); }		
	st_pay_data *pPayData = get_card_pay_data();	
	page_card_showamt(emv_page_win, AppEmvSecondTapCardCallback, get_card_title(), 0, pPayData->amount, pPayData->mode, TIMEOVER,1);
	lv_start_lock(0);

	APP_TRACE("AppEmvSecondTapCard end %d\r\n", g_secondTapRet);

	while(g_secondTapRet == 999)
	{
		MfSdkSysSleep(100);
	}
	
	if(PAGE_RET_TIMEOVR== g_secondTapRet) { iRet = -17; }
	else if(CARD_RET_CANCEL == g_secondTapRet) { iRet = -7; }
	else if(g_secondTapRet < 0) { iRet = -7; }
	else { iRet = g_secondTapRet; }

	APP_TRACE("AppEmvSecondTapCard iRet = %d\r\n", iRet);
...
s32 AppEmvFuncCallback(s32 MsgType,u8* Indata,s32 InLen,u8* OutData,s32* Outlen)
{
	...
	switch (MsgType)
	{
		...
		case MSG_DISPLAY_CARD_NO: 
			...
			break;
		case MFSDK_EMV_MSG_SET_PUREAMT_BEFOREGPO: 
...
void AppInitEmvCallback(void)
{
	...
	MfSdkEmvSetRuPay2ndTapCallback(AppEmvSecondTapCard);
}
