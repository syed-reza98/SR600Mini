...
int saveLogFile(char *buff)
{
	int lenth = -1;
	char szTimeTemp[50] = {0};
	MfSdkSysTime_T t = {0};
	MfSdkSysGetTime(&t);

	int fd = MfSdkFsOpen((const s8*)"exdata//app.log", MFSDK_FS_FLAG_WRITE,MFSDK_FS_MODE_WRITE);
	if(fd != MFSDK_FS_RET_OPEN_FAIL)
	{ 		
		MfSdkFsLseek(fd , 0, MFSDK_FS_SEEK_END);

		sprintf(szTimeTemp, "%d-%d-%d %02d:%02d:%02d ",
		t.nYear, t.nMonth, t.nDay, t.nHour, t.nMinute, t.nSecond);
		MfSdkFsWrite(fd, szTimeTemp, (unsigned int)strlen(szTimeTemp));

		lenth = strlen(buff);
		MfSdkFsWrite(fd, (void*)buff, lenth);
		if (buff[ lenth - 1 ] != '\n')
		{
			MfSdkFsWrite(fd, "\r\n", 2);
		}
