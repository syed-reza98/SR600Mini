...
#define  RECV_LEN 1024
	...
	page_http_update(0, "Connect...");
    m_comm_sock = MfSdkCommSocketCreate(MFSDK_COMM_SOCKET_INDEX_3);

    c_ret = MfSdkCommSocketConnect(m_comm_sock, (s8 *)ip, (s32)port, 3000, NULL);
...
int HttpRequestSet(char *title, char* urlpath, char* SendDate, HttpCallbackFunction callbackfunction)
{	
	...
	page_http_message_show_ex(get_subpage(), _HttpCommStop, title, "Link...", "", "", 60 * 1000, 2);
    return 0;
}
#endif
int HttpRequestFree()
