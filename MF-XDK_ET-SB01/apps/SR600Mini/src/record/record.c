...
{
	int read_bytes = 0;
	int hfile = FILE_OPEN(ORDER_INFO_FILE, FILE_WRITE_FLAG, FILE_READ_MODE);
	int nlen =0;
	...
	if (FILE_OPEN_FAIL != hfile)
	{
		...
		nlen = FILE_LSEEK(hfile, 0, SEEK_END);
		if(nlen>INFO_SIZE*ARR_INFO_MAX+4)
		nlen=INFO_SIZE*ARR_INFO_MAX+4;
		FILE_LSEEK(hfile, 0, SEEK_SET);
		FILE_READ(hfile,&mIndex,4);
		read_bytes = FILE_READ(hfile, order_info, INFO_SIZE*ARR_INFO_MAX);
...
static int AddOrderInfo(st_order_info *OrderInfo)
{
	...
	FILE_DELETE(ORDER_INFO_FILE);
	hfile = FILE_OPEN(ORDER_INFO_FILE, FILE_WRITE_FLAG, FILE_WRITE_MODE);

	if (FILE_OPEN_FAIL != hfile)
	{
		FILE_LSEEK(hfile, 0, SEEK_SET);
		FILE_WRITE(hfile, &mIndex, 4);
		FILE_WRITE(hfile, order_info, INFO_SIZE*ARR_INFO_MAX);
		FILE_CLOSE(hfile);
		mNoDataFlag = 0;
...
void SaveOrderRecord(char *s_data, int len)
{
	...
	if(len < INFO_SIZE)
	{
		info_tmp = MfSdkMemMalloc(INFO_SIZE);
