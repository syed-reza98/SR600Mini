...
int uart_pub_pkt_check_json(unsigned char* pdata, int len, st_pkt_info* pkt)
{
	...
	if (NULL != rootobj)
	{
		uart_pub_get_json(rootobj, (char*)"C", temp, sizeof(temp));
...
int uart_pub_get_json(cJSON* rootobj, char* key, char* val, int size)
{
	...
	if (NULL != itemobj) {
		...
		APP_TRACE("ret = %d, [p_asc:%s]", ret, p_asc);
...
void custom_show_QR_proc(st_pkt_info* pkt_info)
{
	...
	//page_code();
	show_page_qrcode(get_mainpage(),NULL,"",(char*)qrdata,60000);
	state[0] = 0x00;
...
//	unsigned char state[1];
	cJSON* rootobj = 0;
	char* temp = (char*)MfSdkMemMalloc(MAX_LENGTH_1024);
	char szDate[40] = { 0 };
	...
	if (NULL != rootobj)
	{
		...
		memset(temp, 0, MAX_LENGTH_1024);
		uart_pub_get_json(rootobj, "D", temp, MAX_LENGTH_1024);
...
static void uart_set_text(st_pkt_info* pkt_info)
{
	cJSON* rootobj = 0;
	char temp[64] = {0};
	...
	if (NULL != rootobj)
	{
		...
		uart_pub_get_json(rootobj, "T", temp, sizeof(temp));
		strcpy(ET389_qr_data.text, temp);
		APP_TRACE("[%s][%d][text:%s]", __FUNCTION__, __LINE__, ET389_qr_data.text);
...
static void uart_set_rf_power(st_pkt_info* pkt_info)
{
	cJSON* rootobj = 0;
	char temp[64] = {0};
	...
	{
		memset(temp, 0, sizeof(temp));
		uart_pub_get_json(rootobj, "NT", temp, sizeof(temp));
		APP_TRACE("[%s][%d][NT:%s]", __FUNCTION__, __LINE__, temp);
		
		memset(temp, 0, sizeof(temp));
		uart_pub_get_json(rootobj, "RP", temp, sizeof(temp));
		APP_TRACE("[%s][%d][RP:%s]", __FUNCTION__, __LINE__, temp);
		cJSON_Delete(rootobj);
	}
	...
	int rf_power = atoi(temp);
	if(1 == rf_power)
	{
		MfSdkNfcOpen();
		page_text_show(get_mainpage(), "TIP", "power on", 1000);
	}
	else if(2 == rf_power)
	{
		MfSdkNfcClose();
		page_text_show(get_mainpage(), "TIP", "power off", 1000);
	}
}

static void uart_set_rf_exchange(st_pkt_info* pkt_info)
{
	int ret = -1;
	int iRet = -1;
	int tick = 0;
	...
	{
		memset(temp, 0, sizeof(temp));
		uart_pub_get_json(rootobj, "TM", temp, sizeof(temp));
		APP_TRACE("[%s][%d][TM:%s]", __FUNCTION__, __LINE__, temp);
		tick = MfSdkSysTimerOpen(atoi(temp) * 1000);
		
		memset(temp, 0, sizeof(temp));
		uart_pub_get_json(rootobj, "D", temp, sizeof(temp));
		APP_TRACE("[%s][%d][D:%s]", __FUNCTION__, __LINE__, temp);
		cJSON_Delete(rootobj);
	}

	uart_pub_send_pkt_json(pkt_info);

	page_text_show(get_mainpage(), "TIP", "pls use card", 1000);
	while(1)
	{
		if (MfSdkSysTimerCheck(tick) == 0)//timeout
		{
		    MfSdkNfcClose();
			page_text_show(get_mainpage(), "TIP", "timeout!", 3000);
			return;
		}
		
	    iRet = MfSdkNfcDetect();
	    APP_TRACE("MfSdkNfcDetect iRet = %d\r\n", iRet);
	    if (iRet >= 0) 
		{
	        break;
	    }
	...
	MfSdkUtilAsc2Bcd(temp, tempBcd, strlen(temp));
	ret = MfSdkNfcApduStream(tempBcd, strlen(temp)/2, resp, sizeof(resp));
	APP_TRACE("[%s][%d][Apdu resp(%d):%s]", __FUNCTION__, __LINE__, ret, resp);
	if(ret > 0)
	{
		sprintf(msg, "%s", resp);
	}
	...
	{
		sprintf(msg, "fail %d", ret);
	}
	page_text_show(get_mainpage(), "TIP", msg, 3000);
}
...
int uart_func_proc(unsigned char* data, int len, int flag)
{
	...
	else if (MPOS_FUNC_SET_QR == pkt_info.func)
	{
		...
		show_page_qrcode(get_mainpage(), NULL, ET389_qr_data.amt, ET389_qr_data.qrdata, 60000);
	}
	else if (MPOS_FUNC_SET_TEXT == pkt_info.func)
	{
		uart_set_text(&pkt_info);
		page_select_disp(1);
		if(strlen(ET389_qr_data.text) > 0)
			page_text_show(get_mainpage(), "TIP", ET389_qr_data.text, 3000);
	}
	else if (MPOS_FUNC_RF_POWER == pkt_info.func)
	{
		page_select_disp(1);
		uart_set_rf_power(&pkt_info);
	}
	else if (MPOS_FUNC_RF_EXCHANGE == pkt_info.func)
	{
		page_select_disp(1);
		uart_set_rf_exchange(&pkt_info);
	}
	return 0;
}
