...
static lv_obj_t* page_subwin_back = NULL;
...
lv_obj_t* page_show_image_result(lv_obj_t * parent, void* pfunc, char *title, int result, char*msg, int timeout)
{
	char *tip = 0;
	lv_obj_t* img_obj = 0;
	...
	else
	{
		...
		lv_obj_align(page_img, NULL, LV_ALIGN_CENTER, 0, -80);

		if(NULL != tip)
		{
			page_ShowTextOut(page_win, tip, LV_ALIGN_CENTER, 0, 30, LV_COLOR_BLACK, LV_FONT_24);
		}
	...
	page_subwin_back = page_createwin_title(title, LCD_DISP_BEHIND);
	if(page_subwin_back)
	{
		page_select_disp(LCD_DISP_BEHIND);
		page_ShowTextOut(page_subwin_back, tip, LV_ALIGN_CENTER, 0, 0, LV_COLOR_BLACK, LV_FONT_MF12);
	}
...
lv_obj_t* page_show_image(lv_obj_t * parent, void* pfunc, char *title, int result, char*msg, int timeout)
{
	...
	if(MfSdkFsCheckPath((const s8 *)"exdata/success.png") == MFSDK_FS_RET_OK)
	{
		APP_TRACE("success.png exist\r\n");
		return page_show_image_result(parent, pfunc, title, result, msg, timeout);
	}
