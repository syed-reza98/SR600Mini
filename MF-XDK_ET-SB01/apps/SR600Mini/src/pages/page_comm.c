...
static lv_obj_t* lab_message_front = NULL;
...
static void _comm_close_page(int ret)
{
    ...
    if (page_win != NULL) 
	{
		...
		if(NULL != page_subwin)
		{
			...
			if (NULL != lab_message_front)
			{
				lv_obj_del(lab_message_front);
				lab_message_front = 0;
			}
		...
		if(ret == COMM_RET_FAIL_RECV)
			ret = PAGE_RET_RECV_FAIL;
		else if(ret == COMM_RET_FAIL_LINK || ret == COMM_RET_FAIL_CONECT)
			ret = PAGE_RET_CONN_FAIL;
...
static void _comm_message_update(char* title, char* message)
{
	...
	if(lab_message_front)
	{
		lv_label_set_text(lab_message_front, message);
		lv_obj_align(lab_message_front, NULL, LV_ALIGN_CENTER, 0, 90);
	}
}
...
lv_obj_t* page_comm_show_auto(lv_obj_t* parent, void* pfunc, char* title, st_comm_data* comm_data,int mode)
{
	...
	lab_message = page_ShowTextOut(page_win, "Link...", LV_ALIGN_CENTER, 0, 0, LV_COLOR_BLACK, LV_FONT_MF12);
	...
	if(mode)
	{
		...
		if(page_subwin)
		{
			...
			lab_message_front = page_ShowTextOut(page_subwin, "Link...", LV_ALIGN_CENTER, 0, 90, LV_COLOR_BLACK, LV_FONT_24);
			image_task = lv_task_create(load_task_func, 300, LV_TASK_PRIO_LOW, 0);
		}
