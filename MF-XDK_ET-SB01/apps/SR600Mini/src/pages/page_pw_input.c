...
static const char* keybuff[10] = {
	"0.,'?!\"-*@/\\:_;+&%#$=()<>","1qzQZ","2abcABC","3defDEF","4ghiGHI",
...
static void _wifi_input_event_cb(lv_obj_t* obj, lv_event_t e)
{
	...
	if (e == LV_EVENT_KEY) 
	{
        ...
        if (key == MF_LV_KEY_CANCEL_LONG_PRESS)
		{
		...
		if (key >= MF_LV_KEY_1_SHORT_PRESS && key <= MF_LV_KEY_0_TRIPLE_PRESS)
		{
			...
			if(0 == GetKeyStatus(key))//short press
			{
				...
				if((m_keyvalue != keyvalue) || (osl_CheckTimeover(m_input_timeover, 1000) == 1))
				{
					m_keyvalue = keyvalue;
					sprintf(a, "%c", keyvalue);
					lv_textarea_add_text(textarea, a);
					m_input_timeover = MfSdkSysGetTick();
				}
				else
				{
...
static lv_obj_t* mf_create_wifi_input(lv_obj_t* parent, char* msg)
{
	...
	lv_obj_set_size(white_bkg, lv_obj_get_width(parent), lv_obj_get_height(parent));
	...
	lv_obj_set_height(textarea, LINE_HIGHT+8);
	...
	lv_obj_set_style_local_text_font(textarea, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_FONT_MF12);//changyong_18
	lv_obj_align(textarea, white_bkg, LV_ALIGN_IN_TOP_LEFT, INTERVAL, 2*LINE_HIGHT);

	//text:Password
	page_ShowTextOut(white_bkg, "Password:", LV_ALIGN_IN_TOP_LEFT, INTERVAL, LINE_HIGHT, LV_COLOR_BLACK, LV_FONT_MF12);
	...
	lv_obj_align(line, white_bkg, LV_ALIGN_IN_TOP_LEFT, INTERVAL, 3*LINE_HIGHT);
...
lv_obj_t* _mf_wifiinput_show(lv_obj_t* parent, void* pfunc, char* title, char* buff, int maxnum,int timeover)
{
	...
	lv_obj_set_width(wifiname, lv_obj_get_width(white_bkg));
	lv_obj_set_style_local_text_color(wifiname, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_COLOR_BLACK);
	page_show_text_font(wifiname, title, LCD_DISP_NULL);
	lv_obj_align(wifiname, NULL, LV_ALIGN_IN_TOP_MID, 0, 0);

	snprintf(time_set, sizeof(time_set), "(%02ds)", timeover / 1000);
	time_count = page_ShowTextOut(white_bkg, time_set, LV_ALIGN_IN_BOTTOM_MID, 0, 0, LV_COLOR_BLACK, LV_FONT_MF12);
